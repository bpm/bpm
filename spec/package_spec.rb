...
describe BPM::Package, 'InvalidPackagePathError' do
  ...
  it "should raise exception when loading a package with no package.json" do
    FileUtils.mkdir_p home('empty_package')
    package = BPM::Package.new home('empty_package')
    lambda {
      package.load_json
    }.should raise_error(BPM::InvalidPackageError)
  end
